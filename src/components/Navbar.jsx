...
const Logo = () => (
  <Link to="/"
    className="javix-cta inline-flex items-center px-5 py-2.5 rounded-4xl text-sm font-semibold"
...
export const Navbar = () => {
  ...
  return (
    ...
    >
      <nav
        className={`mx-auto max-w-6xl javix-glass rounded-2xl px-4 sm:px-6 py-3 flex items-center justify-between transition-all ${scrolled ? "shadow-lg" : ""
          }`}
        aria-label="Primary"
      >
        <Logo />
          ...
          {links.map((l) => (
            <li key={l.to}>
              <NavLink
                ...
                className={({ isActive }) =>
                  `text-[13px] font-semibold tracking-wider transition-colors ${isActive ? "text-[#4F29B8]" : "text-[#0a1230] hover:text-[#3B82F6]"
                  }`
                }
              >
                {l.label}
            ...
            {links.map((l) => (
              <li key={l.to}>
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `block px-3 py-3 rounded-lg text-sm font-semibold tracking-wider ${isActive
